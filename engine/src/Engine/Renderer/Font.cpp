...
namespace dyxide
{
	...
	Font::Font(const std::string& path) : m_Data(new MSDFData())
	{
		msdfgen::FreetypeHandle* ft = msdfgen::initializeFreetype();
		DYXIDE_ASSERT(ft, "Could not init FreeType Library");

		msdfgen::FontHandle* font = msdfgen::loadFont(ft, path.c_str());
		if (!font)
		{
			DYXIDE_ERROR("Failed to load font at path: " << path);
			return;
		}
		...
		DYXIDE_INFO("Loaded " << glyphsLoaded << " glyphs from font (out of " << charset.size() << ")");
